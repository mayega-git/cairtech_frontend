...
@Component
@ConditionalOnProperty(name = "bbcms.notification.email.provider",
                       havingValue = "brevo")
public class BrevoEmailNotificationAdapter implements EmailSenderPort {

    private static final Logger log =
        LoggerFactory.getLogger(BrevoEmailNotificationAdapter.class);
    ...
    public BrevoEmailNotificationAdapter(
            @Value("${bbcms.notification.email.brevo.api-key}") String apiKey,
    ...
    public Mono<Void> send(List<String> emails, NotificationMessage message) {
        ...
        return webClient.post()
                ...
                .doOnSuccess(resp -> log.debug(
                    "Brevo email sent to {} recipient(s): {}",
                    emails.size(), message.subject()))
                .doOnError(err -> log.warn(
                    "Brevo email send failed for '{}': {}",
                    message.subject(), err.getMessage()))
